...
const buildErrorHtmlPage = (fullDetail: string): string => {
  let parsed: any = {};
  try { parsed = JSON.parse(fullDetail); } catch {}
  ...
  const requestData = parsed.requestData
    ? JSON.stringify(parsed.requestData, null, 2)
    : null;
...
<div class="container">
  ...
  <div class="card">
    ...
    ${response
      ? `<pre>${syntaxHighlight(response)}</pre>`
      : '<div class="empty">Không có dữ liệu phản hồi.</div>'
    }
  </div>
  ...
  <div class="card">
    ...
    ${requestData
      ? `<pre>${syntaxHighlight(requestData)}</pre>`
      : '<div class="empty">Không có request body.</div>'
    }
  </div>
