      ...
      render: (_: any, record: any) => (
        <Flex gap={16} align="center">
          ...
          <Flex vertical>
            ...
            <Text type="secondary" style={{ fontSize: 12 }}>ID sản phẩm: {record.productId}</Text>
          </Flex>
        </Flex>
      ),
    },
      ...
      <Flex align="center" justify="space-between" style={{ marginBottom: 32 }}>
        <Flex vertical gap={4}>
          <Flex align="center" gap={12}>
            <Title level={3} style={{ margin: 0 }}>Mã đơn hàng: #{order.orderId || order.orderCode || order.id}</Title>
            <Tag color={getStatusColor(currentStatus)} bordered={false} style={{ borderRadius: 6, padding: '4px 12px', fontWeight: 600 }}>
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} lg={16}>
          <Space direction="vertical" style={{ width: '100%' }} size={24}>
            ...
            >
              <Title level={5} style={{ padding: '0 24px', marginBottom: 20 }}>Danh sách sản phẩm</Title>
              <Table
                columns={itemColumns}
            ...
            {order.note && (
              <Card
                bordered={false}
                style={{ borderRadius: 20, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', background: '#fafafa' }}
              >
                <Space align="start">
                  <FileTextOutlined style={{ marginTop: 4, color: token.colorPrimary }} />
                  <Flex vertical>
                    <Text strong>Ghi chú khách hàng</Text>
                    <Paragraph style={{ margin: '8px 0 0', color: '#595959' }}>{order.note}</Paragraph>
                  </Flex>
                </Space>
              </Card>
            )}
        ...
        <Col xs={24} lg={8}>
          <Space direction="vertical" style={{ width: '100%' }} size={24}>
            ...
            {/* Payment Info */}
            <Card
               title={<Space><CreditCardOutlined /> Thanh toán</Space>}
               bordered={false}
               style={{ borderRadius: 20, boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}
            >
              <Flex vertical gap={12}>
                ...
                <Flex justify="space-between">
                  ...
                  <Text>{currentStatus === OrderStatus.PAID || currentStatus === OrderStatus.COMPLETED ? formatDate(order.createdAt) : '---'}</Text>
                </Flex>
                <Flex justify="space-between">
            ...
            {isAdmin && (
               <Card
                 ...
                 bordered={false}
                 style={{ borderRadius: 20, boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: `1px solid ${token.colorPrimary}22` }}
               >
                 <Space direction="vertical" style={{ width: '100%' }}>
                   <Button block>Gửi Email cập nhật</Button>
                   <Button block danger>Hủy đơn hàng này</Button>
                 </Space>
               </Card>
            )}
          </Space>
