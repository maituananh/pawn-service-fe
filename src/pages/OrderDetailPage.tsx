...
const OrderDetailPage = () => {
  ...
  const itemColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Text>{formatCurrency(record.price)} VND</Text>
      ),
    },
    {
      title: "Số lượng",
  ...
  return (
    <div
      style={{ padding: "24px 24px 40px", maxWidth: 1100, margin: "0 auto" }}
    >
      {contextHolder}
      ...
      <Breadcrumb
        ...
        items={[
          {
            title: (
              <Link to={isAdmin ? "/admin/dashboard" : "/"}>Trang chủ</Link>
            ),
          },
          {
            title: (
              <Link to={isAdmin ? "/admin/orders" : "/orders"}>Đơn hàng</Link>
            ),
          },
          {
