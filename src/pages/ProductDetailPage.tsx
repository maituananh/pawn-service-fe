...
const ProductDetailPage: React.FC = () => {
  ...
  const { data: relatedPage, isLoading: isLoadingRelated } = useRelatedProducts(
    Number(id),
    1,
    5,
  );
  ...
  const handleAddToCart = () => {
    if (!product) return;
    const avail = product.availableQty ?? product.stockQty;
    if (avail <= 0 || product.status === "SOLD_OUT") {
  ...
  const isOutOfStock = (product.availableQty ?? product.stockQty) <= 0 || product.status === "SOLD_OUT";
  const mainImageUrl =
    selectedImage || product?.images?.[0]?.url || product?.image;
      ...
      <Row gutter={[48, 48]}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: isActive
                          ? `0 0 10px ${token.colorPrimary}40`
                          : "none",
        ...
        <Col xs={24} lg={12}>
          <div className="product-header">
            ...
            >
              {product.description ||
                "Sản phẩm chính hãng với mức giá cạnh tranh nhất trên thị trường. Bảo hành uy tín từ hệ thống Thảo Quyên."}
            </Paragraph>
            ...
            <Descriptions
              ...
              styles={{
                label: { width: 140, fontWeight: 600, background: "#fafafa" },
              }}
            >
              ...
              <Descriptions.Item label="Tình trạng">
                <Badge
                  ...
                  text={
                    !isOutOfStock
                      ? `Còn hàng (${product.availableQty ?? product.stockQty})`
            ...
            <Flex vertical gap={16} style={{ marginBottom: 32 }}>
              <Flex align="center" gap={12}>
                ...
                <InputNumber
                  min={1}
                  max={product.availableQty ?? product.stockQty}
                  value={quantity}
              ...
              <Button
                ...
                style={{
                  ...
                  boxShadow: isOutOfStock
                    ? "none"
                    : `0 8px 20px ${token.colorPrimary}40`,
                }}
