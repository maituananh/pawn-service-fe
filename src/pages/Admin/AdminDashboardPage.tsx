...
const columns = [
  {
    ...
    render: (text: string, record: any) => (
      <Flex vertical gap={2}>
        <Text strong style={{ fontSize: 13 }}>{record.user}</Text>
        <Text type="secondary" style={{ fontSize: 12 }}>{text}</Text>
      </Flex>
    ),
  },
  { title: "Giá trị", dataIndex: "price", key: "price", render: (val: string) => <Text strong color="primary">{val}</Text> },
  ...
  { title: "Sản phẩm", dataIndex: "product", key: "product" },
  {
    ...
    render: (status: string) => (
      <Tag bordered={false} color={statusColors[status]}>{status}</Tag>
    ),
  },
  {
    title: "",
    ...
    <Flex vertical gap={24}>
      ...
      <Flex gap={24} vertical={false} wrap="wrap">
        <Card 
          ...
          title={
            <Flex vertical gap={4}>
              <Title level={5} style={{ margin: 0 }}>Số lượng giao dịch</Title>
              <Text type="secondary" style={{ fontSize: 12, fontWeight: 400 }}>Thống kê theo thời gian</Text>
            </Flex>
          }
          extra={
            <Flex gap={16} align="center">
              ...
              />
              <Button size="small" icon={<ExportOutlined />} type="dashed">Export</Button>
            </Flex>
          }
        >
        ...
        <Card 
          ...
          title={<Title level={5} style={{ margin: 0 }}>Giao dịch gần đây</Title>}
          extra={<Button type="link" size="small">Xem tất cả <RightOutlined /></Button>}
        >
          <Table 
      ...
      <Card 
        ...
        title={
          <Flex vertical gap={4}>
            <Title level={5} style={{ margin: 0 }}>Danh sách cầm đồ đang hoạt động</Title>
            <Text type="secondary" style={{ fontSize: 12, fontWeight: 400 }}>Quản lý các hợp đồng cầm đồ hiện tại</Text>
          </Flex>
        }
      >
        <Table 
