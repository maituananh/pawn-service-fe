...
const AdminCategoryDetailPage = () => {
    ...
    if (isError) {
        return (
            <Alert type="error" message="Không thể tải thông tin danh mục" description={(error as Error)?.message} />
        );
    }
        ...
        <Card title="Thông tin danh mục" extra={<Button onClick={() => navigate(-1)}>Quay lại</Button>}>
            <Form layout="vertical">
                ...
                <div style={{ display: "flex", gap: 12 }}>
                    ...
                    <Button type="primary" onClick={() => navigate(`/admin/categories/edit/${data?.id}`)}>
                        Chỉnh sửa
                    </Button>
                </div>
            </Form>
