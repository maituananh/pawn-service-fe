...
const CartPage = () => {
    ...
    processPayment(
      { productIds: selectedIds },
      {
        onSuccess: (res: any) => {
          ...
          if (checkoutUrl) {
            message.loading(
              "Đang chuyển hướng đến cổng thanh toán Stripe...",
              1.5,
            );
            setTimeout(() => {
  ...
  if (!cart || cart.length === 0)
    return <Empty description="Giỏ hàng trống" style={{ marginTop: 100 }} />;
  ...
  const renderCartItem = (item: CartItem, isInactive = false) => (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        <Text
          type={isInactive ? "secondary" : "danger"}
    ...
    >
      ...
      {inactiveItems.length > 0 && (
        <div style={{ marginBottom: 48 }}>
          ...
          >
            ⚠️ Các sản phẩm này đã bị xoá hoặc ngừng kinh doanh. Vui lòng xoá
            chúng khỏi giỏ hàng.
          </Text>
        </div>
      )}
        ...
        <Space size={32}>
          <div style={{ textAlign: "right" }}>
            <div style={{ color: "#8c8c8c", marginBottom: 4 }}>
              Tạm tính: {formatCurrency(cartTotal)}
            </div>
