...
describe("authApi", () => {
  ...
  it("updateProfile calls /users/{id} with PUT", async () => {
    const id = "1";
    ...
    const result = await authApi.updateProfile(id, payload);

    expect(axiosClient.put).toHaveBeenCalledWith(`/users/${id}`, payload);
    expect(result).toEqual(mockProfile);
  });
